...
function PatientAppointments({ onNotice }) {
  const [state, setState] = useState({ loading: true, error: '', records: [] }); const [cancellingId, setCancellingId] = useState(null);
  const load = async () => { setState({ loading: true, error: '', records: [] }); try { setState({ loading: false, error: '', records: await api('/api/appointments/my/summaries?pageSize=100') }); } catch (error) { setState({ loading: false, error: error.message, records: [] }); } };
  useEffect(() => { load(); }, []);
...
function AccountStatusForm({ onNotice }) {
  const [accounts, setAccounts] = useState([]); const [userId, setUserId] = useState(''); const [isActive, setIsActive] = useState(true); const [error, setError] = useState(''); const [loading, setLoading] = useState(true); const [busy, setBusy] = useState(false);
  useEffect(() => { let active = true; Promise.all([api('/api/admin/staff?pageSize=100'), api('/api/admin/doctors?pageSize=100')]).then(([staff, doctors]) => { if (active) setAccounts([...staff.map((member) => ({ ...member, accountType: 'Staff' })), ...doctors.map((doctor) => ({ ...doctor, jobTitle: doctor.specialization, accountType: 'Doctor' }))]); }).catch((failure) => { if (active) setError(failure.message); }).finally(() => { if (active) setLoading(false); }); return () => { active = false; }; }, []);
