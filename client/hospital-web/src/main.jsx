...
function Page({ role, page, onNotice, onUnauthorized }) {
  ...
  if (role === 'Administrator' && page === 'Doctors') return <><DoctorCreationForm onNotice={onNotice} /><AdministrationProfileManagement key="doctor" type="doctor" onNotice={onNotice} /></>;
...
function BookingForm({ onNotice }) {
  ...
  return <section className="panel">
    ...
    <form className="grid" onSubmit={submit}>
      ...
      {(departmentPage > 1 || hasNextDepartmentPage) && <PaginationControls page={departmentPage} hasNext={hasNextDepartmentPage} onPrevious={() => setDepartmentPage((current) => current - 1)} onNext={() => setDepartmentPage((current) => current + 1)} />}
      ...
      {departmentId && (doctorPage > 1 || hasNextDoctorPage) && <PaginationControls page={doctorPage} hasNext={hasNextDoctorPage} onPrevious={() => setDoctorPage((current) => current - 1)} onNext={() => setDoctorPage((current) => current + 1)} />}
...
function DoctorCreationForm({ onNotice }) {
  const blank = { email: '', password: '', firstName: '', lastName: '', licenseNumber: '', specialization: '', departmentId: '', phoneNumber: '', consultationFee: '', isActive: true };
  const [form, setForm] = useState(blank); const [departments, setDepartments] = useState([]); const [loading, setLoading] = useState(true); const [busy, setBusy] = useState(false); const [error, setError] = useState('');
  useEffect(() => { let active = true; api('/api/admin/departments?pageSize=100').then((records) => { if (active) setDepartments(records.filter((department) => department.isActive)); }).catch((failure) => { if (active) setError(failure.message); }).finally(() => { if (active) setLoading(false); }); return () => { active = false; }; }, []);
  const update = (field) => (event) => setForm((current) => ({ ...current, [field]: field === 'isActive' ? event.target.value === 'true' : event.target.value }));
  const submit = async (event) => { event.preventDefault(); setBusy(true); setError(''); try { await api('/api/admin/doctors', { method: 'POST', body: JSON.stringify({ ...form, departmentId: Number(form.departmentId), consultationFee: Number(form.consultationFee) }) }); setForm(blank); onNotice('Doctor account created. Share the temporary password privately with the doctor.'); } catch (failure) { setError(failure.message); } finally { setBusy(false); } };
  return <section className="panel"><h2>Add doctor</h2><p className="muted">Create a doctor sign-in account and professional profile. The doctor can later update their own contact and professional details.</p>{loading && <p>Loading departments…</p>}{!loading && <form className="grid" onSubmit={submit}><label>Email<input type="email" value={form.email} onChange={update('email')} required maxLength="256" placeholder="doctor@example.com" /></label><label>Temporary password<input type="password" value={form.password} onChange={update('password')} required minLength="8" maxLength="128" autoComplete="new-password" /></label><label>First name<input value={form.firstName} onChange={update('firstName')} required maxLength="100" /></label><label>Last name<input value={form.lastName} onChange={update('lastName')} required maxLength="100" /></label><label>License number<input value={form.licenseNumber} onChange={update('licenseNumber')} required maxLength="100" /></label><label>Specialization<input value={form.specialization} onChange={update('specialization')} required maxLength="150" /></label><label>Department<select value={form.departmentId} onChange={update('departmentId')} required><option value="">Choose an active department</option>{departments.map((department) => <option key={department.departmentId} value={department.departmentId}>{department.departmentCode} — {department.name}</option>)}</select></label><label>Consultation fee<input type="number" min="0" max="1000000" step="0.01" value={form.consultationFee} onChange={update('consultationFee')} required /></label><label>Phone number<input value={form.phoneNumber} onChange={update('phoneNumber')} maxLength="30" /></label><label>Booking availability<select value={String(form.isActive)} onChange={update('isActive')}><option value="true">Active</option><option value="false">Inactive</option></select></label>{error && <p className="error">{error}</p>}<button className="primary" disabled={busy}>{busy ? 'Creating…' : 'Create doctor'}</button></form>}</section>;
}
