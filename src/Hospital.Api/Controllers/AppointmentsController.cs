...
public sealed class AppointmentsController(IAppointmentService appointments) : ControllerBase
{
    ...
    [Authorize(Roles = UserRoles.Patient), HttpGet("appointments/my/summaries")]
    ...
    [Authorize(Roles = UserRoles.Patient), HttpGet("appointments/my/feedback-eligible")]
    public async Task<ActionResult<IReadOnlyList<AppointmentDto>>> FeedbackEligible([FromQuery] PaginationRequest pagination, CancellationToken ct) => Ok(await appointments.GetPatientFeedbackEligibleAppointmentsAsync(UserId, pagination, ct));
    ...
    [Authorize(Roles = UserRoles.Doctor), HttpGet("doctor/appointments/upcoming-work-items")]
    public async Task<ActionResult<IReadOnlyList<DoctorAppointmentWorkItemDto>>> UpcomingWorkItems([FromQuery] PaginationRequest pagination, CancellationToken ct) => Ok(await appointments.GetDoctorUpcomingWorkItemsAsync(UserId, pagination, ct));
